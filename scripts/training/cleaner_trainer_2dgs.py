...
from gsplat.utils_example import (AppearanceOptModule, CameraOptModule,
                                  apply_depth_colormap, colormap, knn,
                                  rgb_to_sh, set_random_seed)
...
from remo_splat.dataset import (Dataset, Parser,
...
class Config:
    ...
    depth_lambda: float = 1e-2 # TODO: Check optimal values
...
def create_splats_with_optimizers(
    ...
    if init_type == "sfm":
        if normals is not None:
            # Initialize the GS quaternions to be the normals
            z_axis = torch.tensor([0.0, 0.0, 1.0], device=normals.device).expand_as(normals)
            dot = (z_axis * normals).sum(dim=-1, keepdim=True)  # (N, 1)
            cross = torch.cross(z_axis, normals, dim=-1)  # (N, 3)
            cross_norm = torch.norm(cross, dim=-1, keepdim=True)

            # Angle between z and n
            angle = torch.acos(torch.clamp(dot, -1.0, 1.0))  # (N, 1)
            half_angle = angle / 2

            # Avoid division by zero
            small_angle = cross_norm < 1e-6
            axis = torch.where(small_angle, torch.tensor([1.0, 0.0, 0.0], device=normals.device), cross / (cross_norm + 1e-8))

            # Quaternion
            w = torch.cos(half_angle)
            xyz = axis * torch.sin(half_angle)
            quats = torch.cat([w, xyz], dim=-1)  # (N, 4)    else:
    else:
        quats = torch.rand((N, 4))  # [N, 4] -> We should initialize with the normal predictions
...
class Runner:
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        render_colors = torch.cat([render_colors, render_median], dim=-1)
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if cfg.scale_reg > 0.0:
                ...
                # )
                scale_large = self.splats["scales"][(torch.exp(self.splats["scales"]) > 0.05).any(dim=1)]
                scale_reg = torch.mean(torch.abs(scale_large))
                loss = loss - cfg.scale_reg * scale_reg
            ...
            if cfg.tb_every > 0 and step % cfg.tb_every == 0:
                ...
                self.writer.add_scalar("train/scale_max", torch.max(torch.exp(self.splats["scales"])).item(), step)
                self.writer.add_scalar("train/scale_min", torch.min(torch.exp(self.splats["scales"][:,:2])).item(), step)
                self.writer.add_scalar("train/scale_mean", torch.mean(torch.exp(self.splats["scales"][:,:2])).item(), step)


                # rr.set_time_sequence("Step", step)
                # rr.log("train/loss", rr.Scalar(loss.item()))
                # rr.log("train/l1loss", rr.Scalar(l1loss.item()))
                # rr.log("train/ssimloss", rr.Scalar(ssimloss.item()))
                # rr.log("train/num_GS", rr.Scalar(len(self.splats["means"])))
                # rr.log("train/mem", rr.Scalar(mem))


                if cfg.depth_loss:
                    self.writer.add_scalar("train/depthloss", depthloss.item(), step)
                    # rr.log("train/depthloss", rr.Scalar(depthloss.item()))
                if cfg.normal_loss:
                    self.writer.add_scalar("train/normalloss", normalloss.item(), step)
                if cfg.dist_loss:
                    self.writer.add_scalar("train/distloss", distloss.item(), step)
                if cfg.tb_save_image and step % cfg.tb_save_image_every == 0:
                    canvas = (
                    ...
                    self.writer.add_images("train/render", canvas, step, dataformats = "NHWC")
